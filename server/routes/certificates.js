...
/* =========================================================
   SIMPLE & RENDER-SAFE PUPPETEER LAUNCH OPTIONS
   ========================================================= */
...
/* =========================================================
   HTML GENERATORS (KEEP YOUR DESIGN LOGIC HERE)
   ========================================================= */
  ...
  <html>
    ...
    <body>
      <div class="container">
        ...
        <div class="name">${doc.studentName}</div>
        <p>${doc.eventDescription || "has successfully completed the program"}</p>
        <div class="footer">
          ${doc.collegeName || "Institution"}
        </div>
      </div>
    </body>
  </html>
...
router.get("/", protect, async (req, res) => {
  try {
    const list = await Certificate.find({ createdBy: req.user._id }).sort({
      createdAt: -1,
    });
    res.json(list);
  } catch (err) {
...
router.get("/:id/download", protect, async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=certificate-${doc._id}.pdf`
    );
    res.send(pdf);
  } catch (err) {
...
router.post("/:id/share", protect, async (req, res) => {
  try {
    ...
    await transporter.sendMail({
      ...
      attachments: [
        {
          filename: `certificate-${doc._id}.pdf`,
          content: pdf,
        },
      ],
    });
